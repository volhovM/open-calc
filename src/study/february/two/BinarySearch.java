...
public class BinarySearch {
    //-----------------------
    // TO_READ: 'right-sorted' means: for each i from {0, mainArray.length - 1} mainArray[i+1] >=
    // mainArray[i];
    ...
    //Post: return = pair(min {k: mainArray[k] >= valueToFind}; number of {k: mainArray[k] ==
    // valueToFind})
    ...
    public static long recursiveMechanismF(long[] mainArray,
        ...
        if (fromIndex >= toIndex - 1) {
            // fromIndex == toIndex - 1
            return fromIndex;
            // (0 <= fromIndex <= valueToFindIndex < toIndex) && !(fromIndex >= toIndex - 1) &&
            // mainArray is 'right-sorted' =>
        ...
        if (mainArray[((int) median)] >= valueToFind) {
            // (0 <= fromIndex <= valueToFindIndex < toIndex - 1) &
            // (fromIndex < median < toIndex) &
            // (mainArray[median] >= valueToFind) => (0 <= fromIndex <= valueToFindIndex < median
            // + 1 < toIndex)
            ...
            // (fromIndex < median < toIndex) &
            // (mainArray[median] < valueToFind) => (0 <= fromIndex < median + 1 <=
            // valueToFindIndex < toIndex)
    ...
    private static long iterativeBinarySearch(long[] mainArray, long valueToFind) {
        ...
        while (fromIndex + 1 < toIndex) {
            ...
            if (mainArray[((int) median)] >= valueToFind) {
                ...
                // (toIndex = median + 1) & (fromIndex <= valueToFindIndex < toIndex) &
                // (mainArray[median] >= valueToFind) =>
                // => (fromIndex <= valueToFind < toIndex) => (fromIndex + 1 <= toIndex)
            } else {
                // fromIndex + 1 < toIndex & fromIndex <= valueToFindIndex < toIndex &
                // fromIndex < median < toIndex - 1 & mainArray[median] < valueToFind
                fromIndex = median + 1;
                // (fromIndex = median + 1) & (fromIndex < median < toIndex - 1) => fromIndex <
                // toIndex
                // (fromIndex < toIndex) & (fromIndex <= valueToFindIndex < toIndex) &
                // (mainArray[median] < valueToFind) =>
                // => (fromIndex <= valueToFindIndex < toIndex) => (fromIndex + 1 <= toIndex)
            }
            //(fromIndex <= valueToFindIndex < toIndex) => valueToFind in [fromIndex; toIndex)
            // #[inv proved]
        }
        // (fromIndex + 1 <= toIndex) & !(fromIndex < toIndex) => fromIndex = toIndex + 1
        // (fromIndex <= valueToFindIndex < toIndex) && (fromIndex = toIndex + 1) && mainArray is
        // 'right-sorted' {non-descending} =>
        // => fromIndex = min (i: mainArray[i] >= valueToFind)
        return fromIndex;
        // return min (i: mainArray[i] >= valueToFind)
    } //////удивительно, но строчка выше похожа на строчку ниже, лол (удивительно в счет текущего
    // ночного времени)
