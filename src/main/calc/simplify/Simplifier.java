...
public class Simplifier {
    ...
    //        public Pair<Integer> containSamePowerVars(Expression expressionOne,
    ...
    private static Expression simplifyInner(Expression expression) {
        ...
        if (expression instanceof BinaryOperations) {
            if (expression instanceof Add) {
                Expression left = ((Add) expression).a = simplify(((Add) expression).a);
                Expression right = ((Add) expression).b = simplify(((Add) expression).b);
                ...
                if ((left instanceof Const) //0 + a = a + 0 -> a
                    ...
                    Expression a = ((left instanceof Const) ? right : left);
                    if (constant.getConstant() == 0) {
                        return a;
                    }
                }
                if (Expression.equalsExp(left, right)) { //a + a -> 2 * a
                    return new Multiply(new Const(2), left);
                }
            } else if (expression instanceof Subtract) {
                Expression left = ((Subtract) expression).a = simplify(((Subtract) expression).a);
                Expression right = ((Subtract) expression).b = simplify(((Subtract) expression).b);
                ...
                if (Expression.equalsExp(left, right)) { //a - a -> 0
                    return new Const(0);
                }
            } else if (expression instanceof Multiply) {
                Expression left = ((Multiply) expression).a = simplify(((Multiply) expression).a);
                Expression right = ((Multiply) expression).b = simplify(((Multiply) expression).b);
                ...
                if (Expression.equalsExp(left, right)) {
                    return new Power(left, new Const(2));
                }
            } else if (expression instanceof Divide) {
                Expression left = ((Divide) expression).a = simplify(((Divide) expression).a);
                Expression right = ((Divide) expression).b = simplify(((Divide) expression).b);
                ...
                if (Expression.equalsExp(left, right)) { //a / a -> 1
                    return new Const(1);
                }
            } else if (expression instanceof Power) {
                Expression left = ((Power) expression).a = simplify(((Power) expression).a);
                Expression right = ((Power) expression).b = simplify(((Power) expression).b);
